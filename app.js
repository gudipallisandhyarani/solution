...
app.get("/user/tweets/feed/", authenticateToken, async (request, response) =>{
    ...
    SELECT
    username, tweet, date_time as dateTime From user INNER JOIN tweet ON user_id = tweet.user_id WHERE user user_id 
...
//API6
app.get("/tweets/:tweetId/", authenticateToken, tweetsAccessVerification,
...
//API7
app.get("/tweets/:tweetId/likes/", authenticateToken, tweetsAccessVerification, async (request, response)=>{
    const {tweetId} = request.params;
...
//API8
app.get("/tweets/:tweetId/replies/", authenticateToken, tweetsAccessVerification,
async (request, response) =>{
    const {tweetId} = request.params;
    ...
    SELECT
    name, reply
    FROM user INNER JOIN reply ON user.user_id =replay.user_id WHERE tweet_id = '${tweetId}';`;
...
app.post("/user/tweets/", authenticateToken, async(request, response)=>{
    ...
    const { userId } =parseInt(request.user_db);
...
//API11
app.delete(
    "/tweets/:tweeId/", authenticateToken, async (request, response) =>{
        const { tweetId } =request.params;
        const { userId } = request;
        const selectUserQuery =`
